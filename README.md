...
- **Three difficulty levels**
  - **Easy** — No opening book, depth-3 fixed search, ~200 ms
  - **Medium** — 16-move perfect opening book, iterative deepening ~1.2 s, perfect solve ≤10 empty
  - **Hard** — 32-move perfect opening book, selectable 5s/10s/15s time limit, perfect solve ≤32 empty
- **Progressive Web App (PWA) Ready** — Fully installable as a standalone mobile app with seamless offline support via a network-first Service Worker cache
...
- **Time-limit safety** — if a 32-ply perfect solve approaches its time budget, the engine bails out and returns the best move found at the previous completed depth, preventing any browser UI freeze
...
- **Transposition table** — 8 MB entries (128 MB RAM) keyed by Zobrist hash with exact/lower/upper bound flags; avoids re-searching known positions across both midgame and perfect-solve phases
...
When the search depth within `negamax` reaches the number of empty squares and the position is within the endgame threshold (≤10 or ≤32 empty, per difficulty), the call is transparently redirected to `negamaxPerfect`. This means iterative deepening seeds the TT with well-ordered move hints before the exact deep search fires — far more efficient than a cold-start 32-ply search.
...
|---|---|---|
| `wasm_init()` | — | — |
| `wasm_getBestMove(cells, isBlack, difficulty, timeLimitMs)` | `int*[64], int, int, int` | cell 0–63, 64=PASS, −1=err |
